...
export default function Industries({ content }: IndustriesProps) {
    ...
    return (
        <section className="py-20 md:py-28 bg-white dark:bg-slate-950 border-t border-slate-900">
            <div className="dynamic-container">
                <div className="text-center mb-16">
                    {isLoading ? (
                        ...
                        >
                            <h2 className="text-3xl md:text-4xl font-bold text-slate-900 dark:text-white mb-4">{content?.header?.title || "Industries We Serve"}</h2>
                ...
                {isLoading ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        {Array.from({ length: 4 }).map((_, i) => (
                            <div key={i} className="p-8 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        {(content?.industries || []).map((item, index) => (
                            <motion.div
                                ...
                                className="group relative overflow-hidden rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-8 hover:bg-slate-100 dark:bg-slate-800 transition-colors"
                            >
                                ...
                                <div className="relative z-10">
                                    ...
                                    <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">{item?.name || "Target Industry"}</h3>
