...
export default function Testimonials({ header, testimonials: initialTestimonials }: TestimonialsProps) {
    ...
    return (
        <section className="py-20 md:py-28 bg-slate-50 dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 relative">
                        ...
                        {isLoading ? (
                            <>
                                {Array.from({ length: 6 }).map((_, i) => (
                                    <div
                                        key={i}
                                        className="flex-shrink-0 w-[350px] p-6 bg-slate-100 dark:bg-slate-800/50 border border-slate-300 dark:border-slate-700 rounded-xl"
                                    >
                                ...
                                {(testimonials || []).length > 0 ? (
                                    [...testimonials, ...testimonials].map((testimonial, index) => (
                                        <div
                                            ...
                                            className="flex-shrink-0 w-[350px] group relative p-6 bg-slate-100 dark:bg-slate-800/50 border border-slate-300 dark:border-slate-700 rounded-xl hover:border-blue-500/50 transition-all duration-300 hover:shadow-lg hover:shadow-blue-900/20"
                                        >
                                            ...
                                            <div className="relative z-10">
                                                ...
                                                {/* Testimonial Text */}
                                                <p className="text-slate-800 dark:text-slate-300 text-sm leading-relaxed mb-4 group-hover:text-slate-900 dark:text-slate-200 transition-colors duration-300">
                                                ...
                                                {/* Author Info */}
                                                <div className="border-t border-slate-300 dark:border-slate-700 pt-4 group-hover:border-blue-500/30 transition-colors duration-300">
                                                    <h4 className="text-slate-900 dark:text-white font-semibold text-sm mb-1 group-hover:text-blue-400 transition-colors duration-300">
                                                        {testimonial?.name || "Customer"}
                                                    </h4>
                                                    <p className="text-slate-700 dark:text-slate-400 text-xs group-hover:text-slate-800 dark:text-slate-300 transition-colors duration-300">
