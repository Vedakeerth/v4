        ...
        const unsubscribe = onAuthStateChanged(auth, (firebaseUser) => {
            if (firebaseUser) {
                ...
                if (firebaseUser.email?.toLowerCase() && adminEmails.includes(firebaseUser.email.toLowerCase())) {
                    navigate("/secure-management-portal/admin");
                }
            }
        });
        return () => unsubscribe();
                ...
                <div className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-8 shadow-2xl">
                    <div className="text-center mb-10">
                        <Link to="/" className="inline-block mb-4">
                            <h1 className="text-3xl font-bold text-slate-900 dark:text-white tracking-widest">VAELÍNSA</h1>
                        </Link>
                    ...
                    <div className="mt-10 text-center pt-6 border-t border-slate-200 dark:border-slate-800">
                        <a href="https://vaelinsa.com" className="text-sm text-slate-500 hover:text-cyan-400 transition-colors">
