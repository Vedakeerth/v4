...
## CreateMovieListItem
You can execute the `CreateMovieListItem` Mutation using the `UseMutationResult` object returned by the following Mutation hook function (which is defined in [dataconnect-generated/react/index.d.ts](./index.d.ts)):
```javascript
useCreateMovieListItem(options?: useDataConnectMutationOptions<CreateMovieListItemData, FirebaseError, CreateMovieListItemVariables>): UseDataConnectMutationResult<CreateMovieListItemData, CreateMovieListItemVariables>;
```
You can also pass in a `DataConnect` instance to the Mutation hook function.
```javascript
useCreateMovieListItem(dc: DataConnect, options?: useDataConnectMutationOptions<CreateMovieListItemData, FirebaseError, CreateMovieListItemVariables>): UseDataConnectMutationResult<CreateMovieListItemData, CreateMovieListItemVariables>;
```
...
```
### Return Type
Recall that calling the `CreateMovieListItem` Mutation hook function returns a `UseMutationResult` object. This object holds the state of your Mutation, including whether the Mutation is loading, has completed, or has succeeded/failed, among other things.

To check the status of a Mutation, use the `UseMutationResult.status` field. You can also check for pending / success / error status using the `UseMutationResult.isPending`, `UseMutationResult.isSuccess`, and `UseMutationResult.isError` fields.

To execute the Mutation, call `UseMutationResult.mutate()`. This function executes the Mutation, but does not return the data from the Mutation.

To access the data returned by a Mutation, use the `UseMutationResult.data` field. The data for the `CreateMovieListItem` Mutation is of type `CreateMovieListItemData`, which is defined in [dataconnect-generated/index.d.ts](../index.d.ts). It has the following fields:
```javascript
...
### Using `CreateMovieListItem`'s Mutation hook function

```javascript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, CreateMovieListItemVariables } from '@dataconnect/generated';
import { useCreateMovieListItem } from '@dataconnect/generated/react'

export default function CreateMovieListItemComponent() {
  // Call the Mutation hook function to get a `UseMutationResult` object which holds the state of your Mutation.
  const mutation = useCreateMovieListItem();

  // You can also pass in a `DataConnect` instance to the Mutation hook function.
  const dataConnect = getDataConnect(connectorConfig);
  const mutation = useCreateMovieListItem(dataConnect);

  // You can also pass in a `useDataConnectMutationOptions` object to the Mutation hook function.
  const options = {
    onSuccess: () => { console.log('Mutation succeeded!'); }
  };
  const mutation = useCreateMovieListItem(options);
  ...
  const mutation = useCreateMovieListItem(dataConnect, options);

  // After calling the Mutation hook function, you must call `UseMutationResult.mutate()` to execute the Mutation.
  // The `useCreateMovieListItem` Mutation requires an argument of type `CreateMovieListItemVariables`:
  ...
  mutation.mutate(createMovieListItemVars);
  // Variables can be defined inline as well.
  ...
  mutation.mutate(createMovieListItemVars, options);
...
export default function UpdateUserProfileComponent() {
  ...
  // Variables can be defined inline as well.
  mutation.mutate({ userId: ..., displayName: ..., email: ..., photoUrl: ..., });
