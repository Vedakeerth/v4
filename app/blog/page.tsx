                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {blogs.map((blog) => (
                        <Link key={blog.id} href={`/blog/${createSeoSlug(blog.title, blog.id)}`} className="group h-full">
                            <article className="bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-3xl overflow-hidden hover:border-cyan-500/50 transition-all duration-500 hover:shadow-2xl hover:shadow-cyan-900/20 flex flex-col h-full active:scale-[0.98]">
                                <div className="relative h-64 overflow-hidden">
                                    ...
                                    <div className="absolute top-4 left-4">
                                        <span className="px-3 py-1.5 rounded-full bg-white dark:bg-slate-950/80 backdrop-blur-md border border-white/10 text-cyan-400 text-[10px] font-extrabold uppercase tracking-[0.2em]">
                                            {blog.category}
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div className="p-8 flex flex-col flex-grow">
                                    ...
                                    <p className="text-slate-600 dark:text-slate-400 text-sm/relaxed mb-6 line-clamp-3">
                                        {blog.excerpt}
                                    </p>
